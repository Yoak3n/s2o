...
ACHIEVEMENT_BASE = 'http://api.steampowered.com/ISteamUserStats/GetPlayerAchievements/v0001/'
...
def get_game_achievement(app_id:str,steam_id:str,key: str):

    uri = ACHIEVEMENT_BASE + '?key=' + key + '&appid=' + app_id + '&steamid=' + steam_id
    headers = {
        'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6,zh-TW;q=0.5',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36 Edg/138.0.0.0'
    }
    ...
    if response is None or response.status_code != 200 :
        if response.status_code == 400:
            return {'playerstats': {'error': 'Requested app has no stats'}}
        else:
            raise Exception(f'Failed to get game achievement for {app_id}: {response.status_code if response else "No response"}')
    return response.json()
